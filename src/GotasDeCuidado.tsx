  ...
  return (
    ...
    <header className="sticky top-0 z-50 border-b border-white/[0.08] bg-gradient-to-r from-slate-800 to-slate-700 shadow-lg shadow-slate-800/20">
      ...
      <div className="mx-auto flex max-w-[1380px] items-center gap-1 sm:gap-2 lg:gap-3 px-3 sm:px-5 lg:px-6 py-2 sm:py-2.5 lg:py-3">
        ...
        {/* Nav Tabs — central, sem fundo */}
        <nav className="hidden md:flex flex-1 min-w-0 items-center justify-center gap-0.5 overflow-x-auto flex-nowrap scrollbar-thin">
          {navItems.map((item) => (
            <button
              key={item.key}
              onClick={() => onNavigate(item.key)}
              className={`group relative flex items-center gap-1 md:gap-1.5 lg:gap-2 px-1.5 md:px-2 lg:px-3 xl:px-4 py-1.5 md:py-1.5 lg:py-2 text-[11px] md:text-[11px] lg:text-xs xl:text-sm font-bold tracking-wide transition-all duration-300 whitespace-nowrap flex-shrink-0 ${
                pagina === item.key
                  ? "text-transparent bg-clip-text bg-gradient-to-r from-bordo-300 to-blue-300 after:absolute after:bottom-0 after:left-2 after:right-2 after:h-[3px] after:rounded-full after:bg-gradient-to-r after:from-bordo-400 after:to-blue-400 after:shadow-lg after:shadow-bordo-500/40 after:animate-pulse after:opacity-80"
                  : "text-white/40 hover:text-white/80"
              }`}
            >
              <span className={`flex-shrink-0 transition-all duration-300 ${pagina === item.key ? "scale-110 drop-shadow-[0_0_6px_rgba(139,92,246,0.3)]" : "group-hover:scale-110 group-hover:drop-shadow-[0_0_4px_rgba(255,255,255,0.15)]"}`}>
                {item.icon}
              </span>

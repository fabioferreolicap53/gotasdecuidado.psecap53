...
export default function HistoricoImportacoes() {
  ...
  // ── Chart: Imports per month ──────────────────────────────────────
  const barOption = useMemo(() => {
    const monthMap: Record<string, { total: number; imported: number; errors: number }> = {};
    for (const log of history) {
      const d = new Date(log.date);
      const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
      if (!monthMap[key]) monthMap[key] = { total: 0, imported: 0, errors: 0 };
      monthMap[key].total++;
      monthMap[key].imported += log.imported;
      monthMap[key].errors += log.errors;
    }
    ...
    return {
      tooltip: {
        ...
        formatter: (params: any) => {
          ...
          let html = `<div style="font-weight:800;margin-bottom:4px">${items[0]?.name || ""}</div>`;
          items.forEach((p: any) => {
            html += `<div style="display:flex;align-items:center;gap:6px;margin-top:2px"><span style="display:inline-block;width:8px;height:8px;border-radius:2px;background:${p.color}"></span><span style="font-size:11px;color:#94a3b8">${p.seriesName}</span><span style="font-weight:900;margin-left:auto">${p.value}</span></div>`;
          });
          return html;
        },
      },
      legend: { bottom: 0, textStyle: { color: "#64748b", fontSize: 10, fontWeight: "bold" }, itemGap: 16, itemWidth: 12, itemHeight: 8, icon: "roundRect" },
      grid: { left: 8, right: 16, bottom: 36, top: 12, containLabel: true },
      xAxis: { type: "category", data: labels, axisLabel: { color: "#64748b", fontSize: 10, fontWeight: "bold" }, axisLine: { lineStyle: { color: "#e2e8f0" } }, axisTick: { show: false } },
      yAxis: { type: "value", minInterval: 1, splitLine: { lineStyle: { color: "#f1f5f9", type: "dashed" } }, axisLabel: { color: "#94a3b8", fontSize: 10 } },
      animationDuration: 1500,
      animationEasing: "elasticOut",
      series: [
        {
          name: "Importados", type: "bar", barWidth: "35%",
          data: sorted.map(([, v]) => v.imported),
        ...
        {
          name: "Erros", type: "bar", barWidth: "35%",
          data: sorted.map(([, v]) => v.errors),
          ...
          animationDelay: (i: number) => i * 100 + 50,
        },
      ],
    };
  }, [history]);
  ...
  const donutOption = useMemo(() => {
    ...
    return {
      ...
      legend: { bottom: 0, textStyle: { color: "#64748b", fontSize: 11, fontWeight: "bold" }, itemGap: 16, itemWidth: 12, itemHeight: 12, icon: "roundRect" },
      animationDuration: 1200,
      animationEasing: "elasticOut",
      series: [{
        type: "pie", radius: ["42%", "70%"], center: ["50%", "44%"],
        itemStyle: { borderRadius: 10, borderColor: "#fff", borderWidth: 3 },
        label: { show: true, position: "outside", fontSize: 13, fontWeight: "900", formatter: "{b}\n{d}%", rich: { b: { fontSize: 11, fontWeight: "bold", color: "#64748b", lineHeight: 18 }, d: { fontSize: 18, fontWeight: "900", color: "#1e293b", lineHeight: 26 } } },
        labelLine: { length: 16, length2: 12, lineStyle: { color: "#cbd5e1", width: 2 }, smooth: true },
        emphasis: { scale: true, scaleSize: 8 },
        animationType: "scale",
        data: [
          { value: successCount, name: "Sucesso", itemStyle: { color: { type: "linear", x: 0, y: 0, x2: 1, y2: 1, colorStops: [{ offset: 0, color: "#10b981" }, { offset: 1, color: "#059669" }] } } },
          { value: partialCount, name: "Parcial", itemStyle: { color: { type: "linear", x: 0, y: 0, x2: 1, y2: 1, colorStops: [{ offset: 0, color: "#f59e0b" }, { offset: 1, color: "#d97706" }] } } },
          { value: errorCount, name: "Erro", itemStyle: { color: { type: "linear", x: 0, y: 0, x2: 1, y2: 1, colorStops: [{ offset: 0, color: "#f43f5e" }, { offset: 1, color: "#e11d48" }] } } },
        ].filter((d) => d.value > 0),
      }],
    };
  }, [history]);

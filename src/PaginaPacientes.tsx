...
import { buscarPacientes, buscarFavoritos, adicionarFavorito, removerFavorito, buscarTodosAcompanhamentos, buscarAcompanhamentos } from "./pocketbase";
...
export default function PaginaPacientes({ usuarioId, onNavigateAcompFiltered }: { usuarioId: string; onNavigateAcompFiltered: (pacienteId: string) => void }) {
                    ...
                    {ultimosAcomps.map((a) => (
                      <div key={a.id} className="px-3 py-2 first:rounded-t-xl last:rounded-b-xl hover:bg-slate-100/50 transition-colors">
                        <div className="flex items-center gap-2.5">
                          ...
                          <span className="inline-flex items-center rounded-full bg-bordo-50 px-1.5 py-px text-[8px] font-bold uppercase tracking-wider text-bordo-700 ring-1 ring-bordo-200/50 whitespace-nowrap">
                            {a.tipo_busca || "—"}
                          </span>
                        </div>
